...
// The ScriptStatsFunc type is an adapter to allow the use of ordinary
// function as ScriptStats mutator.
type ScriptStatsFunc func(context.Context, *ent.ScriptStatsMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ScriptStatsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ScriptStatsMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ScriptStatsMutation", m)
}
